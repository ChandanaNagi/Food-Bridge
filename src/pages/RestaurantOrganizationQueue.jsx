...
export default function RestaurantOrganizationQueue() {
  ...
  async function loadPage() {
    try {
      ...
      console.error('Restaurant organization requests error:', err)
    ...
    () =>
      matches.filter(
        (match) =>
          ...
          match.restaurant_decision === 'pending'
      ),
    [matches]
  )
  ...
  const waiting = useMemo(
    () =>
      matches.filter(
        (match) =>
          match.status === 'pending' &&
          match.restaurant_decision === 'accepted'
      ),
    [matches]
  )
    ...
    () =>
      matches.filter(
        (match) =>
          ...
          match.restaurant_decision === 'declined'
      ),
    [matches]
  )
  ...
  async function acceptMatch(match) {
    try {
      ...
      const bothAccepted = match.shelter_decision === 'accepted'

      const { error: updateError } = await supabase
        .from('organization_matches')
        .update({
          restaurant_decision: 'accepted',
          restaurant_decline_reason: null,
          restaurant_decline_notes: null,
      ...
      setMessage(
        bothAccepted
          ...
          : 'Request accepted. The partnership will become active after the shelter accepts.'
      )
  ...
  async function submitDecline() {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from('organization_matches')
        .update({
          restaurant_decision: 'declined',
          restaurant_decline_reason: declineReason,
          restaurant_decline_notes:
  ...
  return (
    <div style={styles.page}>
      <header style={styles.header}>
        <button style={styles.backButton} onClick={() => navigate('/restaurant')}>
          ← Dashboard
        </button>

        <div>
          <div style={styles.brand}>FoodBridge Detroit</div>
          <div style={styles.role}>Restaurant Portal</div>
        </div>
      ...
      <main style={styles.main}>
        <section style={styles.hero}>
          <div>
            ...
            <p style={styles.subtitle}>
              Review shelters available to partner with{' '}
              <strong>{restaurant?.name || 'your restaurant'}</strong>.
            </p>
          </div>

          <div style={styles.stats}>
            <Stat value={pending.length} label="Needs response" />
            <Stat value={waiting.length} label="Waiting on shelter" />
        ...
        <Section
          ...
          description="Accept a shelter or decline the request and provide a reason."
        >
          {pending.length === 0 ? (
            <Empty text="There are no new shelter requests right now." />
          ) : (
            <div style={styles.grid}>
              {pending.map((match) => (
                <OrganizationCard
                  key={match.id}
                  organization={match.shelters}
                  badge="New request"
                  footer={
        ...
        <Section
          ...
          description="You accepted these requests. They remain pending until the shelter also accepts."
        >
          {waiting.length === 0 ? (
            <Empty text="No partnerships are waiting for shelter approval." />
          ) : (
            <div style={styles.grid}>
              {waiting.map((match) => (
                <OrganizationCard
                  ...
                  footer={
                    <div style={styles.waitingText}>
                      Waiting for shelter acceptance
                    </div>
                  }
                />
              ))}
        ...
        <Section
          ...
          description="These shelters have active partnerships with your restaurant."
        >
          {active.length === 0 ? (
            <Empty text="You do not have any active shelter partners yet." />
          ) : (
            <div style={styles.grid}>
              {active.map((match) => (
                <OrganizationCard
        ...
        {declined.length > 0 && (
          <Section
            ...
            description="Requests declined by your restaurant are shown here for reference."
          >
            <div style={styles.grid}>
              {declined.map((match) => (
                <OrganizationCard
                  ...
                  footer={
                    <div style={styles.reasonBox}>
                      ...
                      {match.restaurant_decline_reason || 'Not provided'}
                      {match.restaurant_decline_reason === 'Other' &&
                        match.restaurant_decline_notes && (
            ...
            <div style={styles.modalHeader}>
              <div>
                ...
                <h2 style={{ margin: '5px 0 0' }}>
                  Decline {declineMatch.shelters?.name || 'this shelter'}?
                </h2>
              </div>
            ...
            {declineReason === 'Other' && (
              <>
                ...
                <textarea
                  ...
                  placeholder="Explain why your restaurant is declining this partnership."
                />
              </>
            )}
    ...
    <article style={styles.card}>
      <div style={styles.cardTop}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <h3 style={styles.cardTitle}>{organization?.name || 'Shelter'}</h3>
        </div>
      </div>
